...
void floyd_warshall_blocked_float(const float* input, float* output, const int n, const int b) {
  ...
  for (int k = 0; k < blocks; k++) {
    floyd_warshall_in_place_float(&output[k*b*n + k*b], &output[k*b*n + k*b], &output[k*b*n + k*b], b, n);
#ifdef _OPENMP
#pragma omp parallel for
#endif
    for (int j = 0; j < blocks; j++) {
      if (j == k) continue;
      floyd_warshall_in_place_float(&output[k*b*n + j*b], &output[k*b*n + k*b], &output[k*b*n + j*b], b, n);
    }
#ifdef _OPENMP
#pragma omp parallel for
#endif
    for (int i = 0; i < blocks; i++) {
      if (i == k) continue;
      floyd_warshall_in_place_float(&output[i*b*n + k*b], &output[i*b*n + k*b], &output[k*b*n + k*b], b, n);
      for (int j = 0; j < blocks; j++) {
	    if (j == k) continue;
	    floyd_warshall_in_place_float(&output[i*b*n + j*b], &output[i*b*n + k*b], &output[k*b*n + j*b], b, n);
      }
    }
  }
}
