...
// expects len(input) == len(output) == n*n
extern "C" void floyd_warshall_blocked(const int *input, int *output, int *parents, const int n, const int b);

#ifdef CUDA
void floyd_warshall_cuda(int* input, int* output, int n);
void floyd_warshall_blocked_cuda(int* input, int* output, int* parents, int n);
#endif
