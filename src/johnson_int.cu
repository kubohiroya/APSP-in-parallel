...
__constant__ graph_cuda_t_int graph_const;
...
__global__ void dijkstra_kernel(int *distanceMatrix, int *successorMatrix, char *visited_global) {
  ...
  edge_t_int *edge_array = graph_const.edge_array;

  int *dist = &distanceMatrix[s * V];
...
__global__ void bellman_ford_kernel(int *dist) {
  ...
  if (e >= E) return;
  int *weights = graph_const.weights;
  edge_t_int *edges = graph_const.edge_array;
  int u = edges[e].u;
  int v = edges[e].v;
...
__host__ bool bellman_ford_cuda(graph_cuda_t_int *gr, int *dist, int s) {
  int V = gr->V;
  int E = gr->E;
  edge_t_int *edges = gr->edge_array;
...
__host__ void johnson_cuda_int(graph_cuda_t_int *gr, int *distanceMatrix, int *successorMatrix) {
  ...
  edge_t_int *device_edge_array;
  int *device_weights;
  int *device_distanceMatrix;
  int *device_successorMatrix;
  int *device_starts;
  // Needed to run dijkstra
  char *device_visited;
  // Allocating memory
  cudaMalloc(&device_edge_array, sizeof(edge_t_int) * E);
  cudaMalloc(&device_weights, sizeof(int) * E);
  cudaMalloc(&device_distanceMatrix, sizeof(int) * V * V);
  ...
  cudaMemcpy(device_edge_array, gr->edge_array, sizeof(edge_t_int) * E,
  ...
  cudaMemcpyToSymbol(graph_const, &graph_params, sizeof(graph_cuda_t_int));
  // End initialization

  graph_cuda_t_int *bf_graph = new graph_cuda_t_int;
  bf_graph->V = V + 1;
  bf_graph->E = gr->E + V;
  bf_graph->edge_array = new edge_t_int[bf_graph->E];
  bf_graph->weights = new int[bf_graph->E];

  std::memcpy(bf_graph->edge_array, gr->edge_array, gr->E * sizeof(edge_t_int));
...
#pragma omp parallel for
  ...
  dijkstra_kernel<<<blocks, THREADS_PER_BLOCK>>>(device_distanceMatrix, device_successorMatrix, device_visited);

  cudaMemcpy(distanceMatrix, device_distanceMatrix, sizeof(int) * V * V, cudaMemcpyDeviceToHost);
