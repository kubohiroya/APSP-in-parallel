...
#ifdef ISPC
extern "C" void floyd_warshall_in_place_float(float* C, const float* A, const float* B, const int b, const int n);
#else
inline void floyd_warshall_in_place_float(float* C, const float* A, const float* B, const int b, const int n) {
  for (int k = 0; k < b; k++) {
    int kth = k*n;
    for (int i = 0; i < b; i++) {
      for (int j = 0; j < b; j++) {
        float sum = A[i*n + k] + B[kth + j];
        if (C[i*n + j] > sum) {
          C[i*n + j] = sum;
        }
      }
