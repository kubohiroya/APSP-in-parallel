...
__global__ void floyd_warshall_block_kernel_phase1_float(int n, int k, float *graph) {
  ...
  __shared__ float C[BLOCK_DIM * BLOCK_DIM];
...
__global__ void floyd_warshall_block_kernel_phase2_float(int n, int k, float *graph) {
  ...
  __shared__ float A[BLOCK_DIM * BLOCK_DIM];
  __shared__ float B[BLOCK_DIM * BLOCK_DIM];
  __shared__ float C[BLOCK_DIM * BLOCK_DIM];
...
__host__ void floyd_warshall_blocked_cuda_float(float *adjancencyMatrix, float *distanceMatrix, int *successorMatrix, int n) {
  ...
  for (int k = 0; k < blocks; k++) {
    floyd_warshall_block_kernel_phase1_float<<<1, block_dim>>>(n, k, device_graph);

    floyd_warshall_block_kernel_phase2_float<<<blocks, block_dim>>>(n, k, device_graph);
...
__host__ void floyd_warshall_cuda_float(float *adjancencyMatrix, float *distanceMatrix, int n) {
